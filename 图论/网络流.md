...
namespace Flow{
    ...
    int n, st, ed, ecnt, head[maxn], dep[maxn];//n为节点数
...
```C++
//上下界网络最大流
int dlt[maxn];//存储每个点 流入-流出
void solve(){
    int n, m, s, t;
    cin >> n >> m >> s >> t;
    //s, t是源汇， n+1, n+2是差量网络上的虚拟源，汇

    Flow::init(n+1, n+2, n+2);
    Flow::add(t, s, 1e18);//t->s 连[0, inf)的流
    int ets = Flow::ecnt;//记录链接t->s的边号，一会要删
    for(int i=1;i<=m;i++){
        int x, y, wi, wx;
        cin >> x >> y >> wi >> wx;
        Flow::add(x, y, wx - wi);//建立差量网络
        dlt[x] -= wi;
        dlt[y] += wi;
    }
    ll req = 0;//计算虚拟源的的总流出量
    for(int i=1;i<=n;i++){
        if(dlt[i] > 0) Flow::add(n+1, i, dlt[i]), req += dlt[i];
        if(dlt[i] < 0) Flow::add(i, n+2, -dlt[i]);
    }
    if(Flow::getFlow() != req){
    ...
    //上一部分得到了一个可行流，这个可行流保证了下届，其流量即为t->s边的流量（并不是虚拟源汇点间的流量！）
    ll ans = Flow::e[ets].w;
    //切换源汇，删边
    Flow::st = s, Flow::ed = t;
    Flow::e[ets].w = 0;
...
如果是无源汇判断可行流，就不需要添加 $t->s$  的连边，跑到return处判完了即可
...
void solve(){
    ...
    Flow::init(n+2, n+3, n+3);
    Flow::add(n+1, 1, 1e18, 0);//t->s 连[0, inf)的边，附加边费用全为0
    ll res = 0;
    for(int i=1;i<=n;i++){
        Flow::add(i, n+1, 1e18, 0);//连[0, inf)的边
        int m; cin >> m;
        for(int j=0;j<m;j++){
            int t, c; cin >> t >> c;
            Flow::add(i, t, 1e18 - 1, c);//这里连[1, inf)的边
            res += c;
            dlt[i] -= 1;
            dlt[t] += 1;
        }
    ...
    ll req = 0;//计算虚拟源的的总流出量
    for(int i=1;i<=n+1;i++){
        if(dlt[i] > 0) Flow::add(n+2, i, dlt[i], 0), req += dlt[i];
        if(dlt[i] < 0) Flow::add(i, n+3, -dlt[i], 0);
    }
