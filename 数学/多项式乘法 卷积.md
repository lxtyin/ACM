...
namespace Poly {
    ...
    int ta[maxn << 2], tb[maxn << 2]; //两个临时数组 a[0..n] * b[0..m]，结果存入ta
    void multipy(int* a, int n, int* b, int m) { //此处不改变a, b数组，卡常可以改为原地修改
        int lim = 1;
        while(lim <= n + m) lim *= 2; //取大于n+m的次幂（a_0...n，变换时是0...lim-1共2的次幂位数
        calcurev(lim);
        ...
        for(int i = 0; i <= n + m; i++) ta[i] = (ll)ta[i] * inv % P; //结果存入ta，卡常可直接写回a
    }
};
```
